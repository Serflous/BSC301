...
public class JSONParse {
    ...
    public List<Phobia> parseDictionary(String in){
        ...
        for (int i = 0; i < al.size(); i++){
            Phobia ph = new Phobia();
            ph.name = (String) ( (Map) al.get(i) ).get("phobia");
            ph.description = (String) ( (Map) al.get(i) ).get("desc");
                      
            
            List<String> wordsList = (List<String>) ( (Map) al.get(i) ).get("words");
            
            for (int j = 0; j < wordsList.size(); j++) {
                ph.synonyms.add( (String) wordsList.get(j) );
            }
            phobias.add(ph);
        }
        return phobias;
